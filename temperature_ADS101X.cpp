...
{
    i2cDriverCommandSetup(i2c_adc_setup_command, ADS101X_ADC_ADDRESS << 1 | I2C_WRITE_BIT, I2C_QUEUE_PRIO_HIGH, i2c_adc_setup_buffer, sizeof(i2c_adc_setup_buffer));
    i2cDriverCommandSetup(i2c_adc_pointer_command, ADS101X_ADC_ADDRESS << 1 | I2C_WRITE_BIT, I2C_QUEUE_PRIO_HIGH, i2c_adc_pointer_buffer, sizeof(i2c_adc_pointer_buffer));
    i2cDriverCommandSetup(i2c_adc_read_command, ADS101X_ADC_ADDRESS << 1 | I2C_READ_BIT, I2C_QUEUE_PRIO_HIGH, i2c_adc_read_buffer, sizeof(i2c_adc_read_buffer));
...
bool temperatureADS101XReady()
{
    ...
    //Check for an read error
    if (i2c_adc_read_buffer[0] == 0xFF && i2c_adc_read_buffer[1] == 0xFF)
    {
        //If we have a single or dual read error, do a retry the read. Else raise an error.
        if (error_counter < 3)
        {
