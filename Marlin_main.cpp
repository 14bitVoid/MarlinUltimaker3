...
// G32 - set bed leveling factors
// G90 - Use Absolute Coordinates
// G91 - Use Relative Coordinates
// G92 - Set current position to coordinates given
// G93 - Set the distance between the tip of the filament and the hotend.
...
// M301 - Set nozzle PID parameters FF, P, I, D, i, C, E and R
// M302 - Allow cold extrudes, or set the minimum extrude S<temperature>
// M304 - Set bed PID parameters FF, P, I, D, i, C, E and R
...
void process_commands()
{
  if(code_seen('G'))
  {
    ...
    case 30: // G30 - Probe Z at current position and report result.
      {
        int probe_feedrate = 24; // speed (mm/min)
        ...
        destination[Z_AXIS] = probeWithCapacitiveSensor(destination, probe_feedrate, verbosity, move_distance, extra_z_move);
    ...
    case 31: // G31 - get/update capacitive sensor base level
      ...
      break;
    case 32: // G32 - set bed leveling factors
      {
        if (code_seen('X')) planner_bed_leveling_factor[X_AXIS] = code_value();
        if (code_seen('Y')) planner_bed_leveling_factor[Y_AXIS] = code_value();
      }
      break;
#endif
    ...
    case 92: // G92 - Set current position to coordinates given
        ...
        if(code_seen(axis_codes[i]))
        {
          if(i == E_AXIS)
          {
             ...
             for(uint8_t e=0; e<EXTRUDERS; e++)
                hotend_pid[e].updateExtrusionPosition();
             heated_bed_pid.updateExtrusionPosition();
             if (active_extruder == 0)
             {
               initFlowPosition(0);
             }
      ...
      break;
    case 93://G93 - Set the distance between the tip of the filament and the hotend tip.
      {     //      This is used for the Ke factor of the PID controller, which needs to know when material is extruded from the hotend.
        float offset = 0;
        if (code_seen(axis_codes[E_AXIS]))
            offset = code_value();
        for(uint8_t e=0; e<EXTRUDERS; e++)
            hotend_pid[e].resetExtrusionPosition(offset);
        heated_bed_pid.resetExtrusionPosition(offset);
      }
      break;
    }
  }
  ...
  else if(code_seen('M'))
  {
    ...
    case 149: // M149 T[0-1] - Read hotend SERIAL
      {
        ...
        {
          SERIAL_PROTOCOLPGM("SERIAL:");
          for(uint8_t n = 0; n < DS2431_SERIAL_SIZE; n++)
          {
            if (serial[n] < 16)
              MYSERIAL.print('0');
            MYSERIAL.print(serial[n], HEX);
          }
          MYSERIAL.println();
        }
        else
        {
          SERIAL_PROTOCOLLNPGM("NO-HOTEND");
        }
      }
      break;
    case 150: // M150 T[0-1] P[0-3] - Read raw hotend eeprom page as hex data.
      {
        ...
        {
          SERIAL_PROTOCOLLNPGM("PAGE INDEX OUT OF RANGE");
          break;
        }
        ...
        if (oneWireDS2431Read(index, page * sizeof(data), data, sizeof(data)))
        {
          ...
          SERIAL_PROTOCOLPGM("DATA: 0x");
          MYSERIAL.println(hex_data);
        }
        else
        {
          SERIAL_PROTOCOLLNPGM("NO-HOTEND");
    ...
    case 151: // M151 T[0-1] A<register address> D<hex data(16 characters/nibbles)> - Write Raw hotend data per 8 bytes.
    {
        ...
        {
            SERIAL_PROTOCOLLNPGM("WARNING: NO TOOL PARAMETER FOUND");
            break;
        }
        if (code_seen('A'))
        {
        ...
        {
            SERIAL_PROTOCOLLNPGM("WARNING: NO ADDRESS PARAMETER FOUND");
            break;
        }
        if (code_seen('D'))
        {
        ...
        {
            SERIAL_PROTOCOLLNPGM("WARNING: NO DATA PARAMETER FOUND");
            break;
        }

        if (hex2bin(chunk, hex_data, sizeof(chunk)) == sizeof(chunk) * 2)
        {
            SERIAL_PROTOCOLLNPGM("WRITING");
            if (oneWireDS2431WritePage(index, address, chunk))
            {
                SERIAL_PROTOCOLLNPGM("DATA_WRITTEN");
            }
            else
            {
                SERIAL_PROTOCOLLNPGM("ERROR: DATA_NOT WRITTEN");
            }
        }
        else
        {
            SERIAL_PROTOCOLLNPGM("WARNING: NO (OR NOT ENOUGH) DATA FOUND");
        }
        break;
    }
    ...
    case 140: // M140 - Set bed target temp
      if (code_seen('S')) heated_bed_pid.setTargetTemperature(code_value());
      break;
    case 105 : // M105 - Read current temp
        ...
        for(tmp_extruder=0; tmp_extruder<EXTRUDERS; tmp_extruder++)
        {
            ...
            SERIAL_PROTOCOL('/');
            SERIAL_PROTOCOL_F(hotend_pid[tmp_extruder].getTargetTemperature(), 1);
            SERIAL_PROTOCOL('@');
            if (heater_output_accumulator_counter[tmp_extruder] > 0)
            {
                SERIAL_PROTOCOL(heater_output_accumulator[tmp_extruder] / heater_output_accumulator_counter[tmp_extruder]);
            ...
            SERIAL_PROTOCOL('p');
            SERIAL_PROTOCOL(hotend_present[tmp_extruder]);
            SERIAL_PROTOCOL('e');
            SERIAL_PROTOCOL(!allow_temperature_sensor_errors[tmp_extruder]);
            SERIAL_PROTOCOL('f');
            SERIAL_PROTOCOL(flow_sensor_raw[tmp_extruder]);
            SERIAL_PROTOCOL('/');
            SERIAL_PROTOCOL(flow_sensor_e_position[tmp_extruder]);
        }
        #if defined(TEMP_BED_PIN) && TEMP_BED_PIN > -1
        SERIAL_PROTOCOLPGM(" B");
        SERIAL_PROTOCOL_F(degBed(), 1);
        SERIAL_PROTOCOL('/');
        SERIAL_PROTOCOL_F(heated_bed_pid.getTargetTemperature(), 1);
        SERIAL_PROTOCOL('@');
        if (bed_heater_output_accumulator_counter > 0)
        {
            SERIAL_PROTOCOL(bed_heater_output_accumulator / bed_heater_output_accumulator_counter);
      ...
      case 81: // M81  - Turn off Power Supply (ATX Power Off)
        ...
        st_synchronize();
        physicallyPowerDownMotorsAndHeaters();
        break;
    ...
    case 119: // M119 - Output Endstop status to serial port
        ...
        SERIAL_PROTOCOL(((READ(X_MIN_PIN) ^ X_ENDSTOPS_INVERTING) ? MSG_ENDSTOP_HIT : MSG_ENDSTOP_OPEN));
        SERIAL_PROTOCOL(" ");
#endif
#if (X_MAX_PIN > -1)
        SERIAL_PROTOCOLPGM(MSG_X_MAX);
        SERIAL_PROTOCOL(((READ(X_MAX_PIN) ^ X_ENDSTOPS_INVERTING) ? MSG_ENDSTOP_HIT : MSG_ENDSTOP_OPEN));
        SERIAL_PROTOCOL(" ");
#endif
#if (Y_MIN_PIN > -1)
        SERIAL_PROTOCOLPGM(MSG_Y_MIN);
        SERIAL_PROTOCOL(((READ(Y_MIN_PIN) ^ Y_ENDSTOPS_INVERTING) ? MSG_ENDSTOP_HIT : MSG_ENDSTOP_OPEN));
        SERIAL_PROTOCOL(" ");
#endif
#if (Y_MAX_PIN > -1)
        SERIAL_PROTOCOLPGM(MSG_Y_MAX);
        SERIAL_PROTOCOL(((READ(Y_MAX_PIN) ^ Y_ENDSTOPS_INVERTING) ? MSG_ENDSTOP_HIT : MSG_ENDSTOP_OPEN));
        SERIAL_PROTOCOL(" ");
#endif
#if (Z_MIN_PIN > -1)
        SERIAL_PROTOCOLPGM(MSG_Z_MIN);
        SERIAL_PROTOCOL(((READ(Z_MIN_PIN) ^ Z_ENDSTOPS_INVERTING) ? MSG_ENDSTOP_HIT : MSG_ENDSTOP_OPEN));
        SERIAL_PROTOCOL(" ");
#endif
#if (Z_MAX_PIN > -1)
        SERIAL_PROTOCOLPGM(MSG_Z_MAX);
        SERIAL_PROTOCOL(((READ(Z_MAX_PIN) ^ Z_ENDSTOPS_INVERTING) ? MSG_ENDSTOP_HIT : MSG_ENDSTOP_OPEN));
        SERIAL_PROTOCOL(" ");
#endif
        SERIAL_PROTOCOLLN("");
        break;
    ...
    case 301: // M301 - Set nozzle PID parameters FF, P, I and D
      {
        ...
        if (code_seen('E')) hotend_pid[tmp_extruder].setKe(code_value(), tmp_extruder);
      }
      break;
    ...
    break;
    case 12002: //M12002 - Set flow sensor feature enable/disable
    {
        if (code_seen('S'))
        {
            flow_sensor_enabled = code_value();
...
void get_coordinates()
{
    for(int8_t i=0; i < NUM_AXIS; i++)
    {
        if(code_seen(axis_codes[i]))
        {
            ...
            //This is a extra safety check against corrupted GCode.
            if (i < E_AXIS)
            {
...
void Stop(uint8_t reasonNr)
{
    ...
    if(Stopped == false)
    {
        ...
        case STOP_REASON_MAXTEMP(0):
            SERIAL_ERRORLNPGM("EXTRUDER_MAXTEMP_ERROR:0: Extruder switched off. MAXTEMP triggered !");
            break;
        case STOP_REASON_MAXTEMP(1):
            SERIAL_ERRORLNPGM("EXTRUDER_MAXTEMP_ERROR:1: Extruder switched off. MAXTEMP triggered !");
            break;
        case STOP_REASON_MINTEMP(0):
            SERIAL_ERRORLNPGM("EXTRUDER_MINTEMP_ERROR:0: Extruder switched off. MINTEMP triggered !");
            break;
        case STOP_REASON_MINTEMP(1):
            SERIAL_ERRORLNPGM("EXTRUDER_MINTEMP_ERROR:1: Extruder switched off. MINTEMP triggered !");
            break;
        case STOP_REASON_MAXTEMP_BED:
            SERIAL_ERRORPGM("BED_MAXTEMP_ERROR: Temperature heated bed switched off. MAXTEMP triggered !!");
            break;
        case STOP_REASON_HEATER_ERROR(0):
            SERIAL_ERRORLNPGM("HEATER_ERROR:0");
            break;
        case STOP_REASON_HEATER_ERROR(1):
            SERIAL_ERRORLNPGM("HEATER_ERROR:1");
            break;
        ...
        case STOP_REASON_I2C_COMM_ERROR:
            SERIAL_ERRORLNPGM("I2C_COMM_ERROR");
            break;
        case STOP_REASON_SAFETY_TRIGGER:
            SERIAL_ERRORLNPGM("SAFETY_ERROR: Safety circuit triggered");
            break;
        case STOP_REASON_SERIAL_INPUT_TIMEOUT:
            SERIAL_ERRORLNPGM("SERIAL_INPUT_TIMEOUT: No commands received over time. Safety shutdown");
            break;
        case STOP_REASON_POSITION_ERROR:
            SERIAL_ERRORLNPGM("POSITION_ERROR: Ordered to move outside of normal volume.");
            break;
